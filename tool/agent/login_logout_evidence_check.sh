...
usage() {
  cat >&2 <<'EOF'
Usage:
  tool/agent/login_logout_evidence_check.sh --device <id> [Maestro runner options]

Environment:
  FIXTURE_API_BASE_URL  Backend API root (default: http://localhost:4000/v1).

The wrapper provisions a run-scoped account, runs the login/logout Maestro
flow, revokes all sessions, and removes generated credentials from text
artifacts. Do not pass --flow; this wrapper selects the flow.
EOF
}
...
for ((index = 0; index < ${#runner_args[@]}; index++)); do
  case "${runner_args[$index]}" in
    --flow) fail 'Do not pass --flow to the login/logout wrapper.' ;;
    --artifacts-dir)
      ((index + 1 < ${#runner_args[@]})) || fail '--artifacts-dir requires a value.'
      artifacts_dir="${runner_args[$((index + 1))]}"
      ((index += 1))
      ;;
    --device)
      ((index + 1 < ${#runner_args[@]})) || fail '--device requires a value.'
      device_id="${runner_args[$((index + 1))]}"
      ((index += 1))
      ;;
    -h|--help) usage; exit 0 ;;
  esac
...
if [[ -z "$artifacts_dir" ]]; then
  artifacts_dir="_artifacts/mobile/login-logout-$(date '+%Y%m%d_%H%M%S')"
...
fixture_email="maestro-login-$(date +%s)-$RANDOM@example.test"
