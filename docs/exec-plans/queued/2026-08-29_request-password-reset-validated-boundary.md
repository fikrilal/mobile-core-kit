...
- [ ] Migrate presentation/use-case input to one raw email `String` and the repository to `EmailAddress`.
- [ ] Map `EmailAddress` in `PasswordResetRequestModel` and remove the old entity.
- [ ] Update focused tests and integration fakes.
...
Exercise invalid and successful reset requests on a supported mobile target,
without asserting whether an account exists. Record sanitized evidence.
