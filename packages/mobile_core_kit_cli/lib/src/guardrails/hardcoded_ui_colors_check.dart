...
class HardcodedUiColorsCheck {
  const HardcodedUiColorsCheck({
  ...
  int run({String target = 'lib'}) {
    ...
    if (violations.isEmpty) {
      output.writeln('OK: no hardcoded UI colors found.');
      return 0;
    }

    errorOutput.writeln('Hardcoded UI colors found.\n');
    errorOutput.writeln(
      'Rule: UI code MUST consume roles, not hardcoded colors.',
    );
    errorOutput.writeln(
    ...
    for (final v in violations) {
      errorOutput.writeln(
        '- ${v.path}:${v.line}:${v.column} → ${v.patternName}',
      );
