...
We regularly see single Codex runs work on a single task for upwards of six hours (often while humans are sleeping).
...
The diagram in the original article shows the rule: within each business domain (e.g. App Settings), code can only depend “forward” through a fixed set of layers (Types -> Config -> Repo -> Service -> Runtime -> UI). Cross-cutting concerns (auth, connectors, telemetry, feature flags) enter through a single explicit interface: Providers. Anything else is disallowed and enforced mechanically.
...
In practice, these rules are enforced with custom linters and structural tests, plus a small set of “taste invariants.” For example, the team statically enforces structured logging, naming conventions for schemas and types, file size limits, and platform-specific reliability requirements with custom lints. Error messages include remediation instructions to help the agent self-correct.
...
At the same time, the team is explicit about where constraints matter and where they do not. This resembles leading a large engineering platform organization: enforce boundaries centrally, allow autonomy locally. Boundaries, correctness, and reproducibility are strict; expression within boundaries is flexible.
...
Human taste is fed back continuously. Review comments, refactoring pull requests, and user-facing bugs are captured as documentation updates or encoded directly into tooling. When documentation falls short, the rule is promoted into code.
...
When the team says the codebase is generated by Codex agents, they mean everything in the codebase.
...
Humans always remain in the loop, but work at a different layer of abstraction than before. They prioritize work, translate user feedback into acceptance criteria, and validate outcomes. When the agent struggles, this is treated as a signal to add missing tools, guardrails, or documentation, and have Codex write that improvement.

Agents use standard development tools directly. They pull review feedback, respond inline, push updates, and often squash and merge their own pull requests.

## Increasing levels of autonomy

As more of the development loop was encoded directly into the system—testing, validation, review, feedback handling, and recovery—the repository recently crossed a threshold where Codex can end-to-end drive a new feature.
...
This behavior depends heavily on the repository’s structure and tooling and should not be assumed to generalize without similar investment.
...
Initially, humans addressed this manually. The team reportedly spent every Friday (20% of the week) cleaning up “AI slop,” which did not scale.

Instead, they encoded “golden principles” directly into the repository and built a recurring cleanup process. These are opinionated, mechanical rules that keep the codebase legible and consistent for future agent runs. Examples given include preferring shared utility packages over hand-rolled helpers and validating boundaries or using typed SDKs rather than guessing data shapes.

On a regular cadence, background Codex tasks scan for deviations, update quality grades, and open targeted refactoring pull requests. Most can be reviewed quickly and automerged.

This functions like garbage collection. Technical debt is like a high-interest loan: paying it down continuously in small increments is usually better than allowing it to compound and then tackling it in large bursts. Human taste is captured once, then enforced continuously on every line of code.

## What we’re still learning

This strategy has worked through internal launch and adoption at OpenAI. Building a real product for real users helped anchor investments in reality and guide toward long-term maintainability.

Open questions remain, including how architectural coherence evolves over years in a fully agent-generated system, where human judgment adds the most leverage, and how to encode that judgment so it compounds. It is also unclear how this system will evolve as models become more capable.

What has become clear: building software still demands discipline, but the discipline increasingly lives in scaffolding rather than hand-written code. Tooling, abstractions, and feedback loops that keep a codebase coherent become increasingly important.

The hardest challenges center on designing environments, feedback loops, and control systems that help agents build and maintain complex, reliable software at scale.

As agents like Codex take on larger portions of the software lifecycle, these questions will matter even more.

## Acknowledgements

Special thanks to Victor Zhu and Zach Brock, as well as the team that built the product.
